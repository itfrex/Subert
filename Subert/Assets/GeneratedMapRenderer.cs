...
public class GeneratedMapRenderer : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        //GetComponent<MeshRenderer>().material.mainTexture = GenerateChunk(0, 0);
        UpdateChunks(0, 0);
    }
    ...
    void FillCells()
    {
        for (int y = 0; y < arraySize; y++)
        {
            for (int x = 0; x < arraySize; x++)
            {
                cells[x][y] = World.world.CheckCollision(x, y);
            }
        }
    }
    ...
    void GenerateCorners()
    {
        for(int y = 0; y < arraySize; y++)
        {
            for (int x = 0; x < arraySize; x++)
            {
                if (cells[x][y])
                {
                    corners[x][y] = Mathf.RoundToInt(Mathf.PerlinNoise(x * roughness, y * roughness) * maxVariance);
                    corners[x+1][y] = Mathf.RoundToInt(Mathf.PerlinNoise((x+1) * roughness, y * roughness) * maxVariance);
                    corners[x][y +1] = Mathf.RoundToInt(Mathf.PerlinNoise(x * roughness, (y+1) * roughness) * maxVariance);
                    corners[x+1][y +1] = Mathf.RoundToInt(Mathf.PerlinNoise((x+1) * roughness, (y+1) * roughness) * maxVariance);
                    edges[x][2 *y] = true;
                    edges[x][2 *y+1] = true;
                    edges[x][2 *y+2] = true;
                    edges[x + 1][2 *y + 1] = true;
    ...
    void GenerateMiniTexture(int x, int y)
    {
        if (cells[x][y])
        {
            FillSquare(0, 0);
        ...
        else
        {
            ...
            else if (numCorners == 2)
            {
                if (edges[x][2 * y])
                {
                    DrawOneEdge(0, 0, 2, corners[x][y], corners[x + 1][y]);
                }
                else if (edges[x][2 * y + 1])
                {
                    DrawOneEdge(0, 0, 1, corners[x][y], corners[x][y + 1]);
                }
                else if (edges[x][2 * y + 2])
                {
                    DrawOneEdge(0, 0, 0, corners[x][y + 1], corners[x + 1][y + 1]);
                }
                else if (edges[x + 1][2 * y + 1])
                {
                    DrawOneEdge(0, 0, 3, corners[x + 1][y], corners[x + 1][y + 1]);
                }
                else
                {
                    DrawCorners(0, 0, x, y);
            ...
            else if (numCorners == 3)
            {
                ...
                if (edges[x][2 * y] && edges[x][2 * y + 1])
                {
                    DrawCurve(0, 0, 0, 0, corners[x][y + 1], corners[x + 1][y]);
                }
                //DR
                else if (edges[x][2 * y] && edges[x + 1][2 * y + 1])
                {
                    DrawCurve(0, 0, pixelsPerTile - 1, 0, corners[x + 1][y + 1], corners[x][y]);
                }
                //UL
                else if (edges[x][2 * y + 2] && edges[x][2 * y + 1])
                {
                    DrawCurve(0, 0, 0, pixelsPerTile - 1, corners[x][y], corners[x + 1][y + 1]);
                }
                //UR
                else if (edges[x][2 * y + 2] && edges[x + 1][2 * y + 1])
                {
                    DrawCurve(0, 0, pixelsPerTile - 1, pixelsPerTile - 1, corners[x + 1][y], corners[x][y + 1]);
                }
                //D
                else if (edges[x][2 * y])
                {
            ...
            else if (numCorners == 4)
            {
                ...
                else if (edges[x][2 * y + 2] && edges[x][2 * y])
                {
                    ...
                    {
                        DrawCurve(0, 0, pixelsPerTile - 1, 0, corners[x][y], corners[x][y + 1]);
                        DrawCurve(0, 0, pixelsPerTile - 1, pixelsPerTile - 1, corners[x + 1][y], corners[x][y + 1]);
                    }
                    //OLOR
                    else
